...
- **gate** vs **monitor** — a *gate* decides one SKU-day, before the order
  is shown (answer or abstain). A *monitor* watches many SKU-days after the
  fact and raises an alert when performance drifts (e.g. systematic
  over-ordering, intervals missing too often). Gates protect one decision;
  monitors catch problems no single decision reveals.
- **leak** — any information the model sees that it could not have had at
...
- **units over / units short** — units ordered beyond demand (waste) and
  demand not covered (stockout). A case count can match and still waste
  units, so these sit beside case errors.
- **demo data** — a small synthetic sales series, generated by a committed
  script, that lets the pipeline run without the Kaggle data. Never used for
  any reported result.
